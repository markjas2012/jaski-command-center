...
export default function PhishFeature() {
  ...
  const flatSongs = latest?.songs || [];
  const stories = feed?.news?.slice(0, 3) || [];
  const totalSongs = sets.length
    ? sets.reduce((sum, set) => sum + set.songs.length, 0)
    : flatSongs.length;
  const fallbackLinks = [
      ...
      <div className={styles.grid}>
        <article className={`${styles.card} ${styles.primary}`}>
          ...
          <div className={styles.locationLine}>
            {latest?.location || "Location unavailable — open the full setlist for show details."}
          </div>
          <div className={styles.showMeta}>
            <span>{totalSongs ? `${totalSongs} songs` : "Setlist ready when published"}</span>
            <span>{sets.length ? `${sets.length} sections` : "Phish.net"}</span>
          </div>
          <div className={styles.links}>
            <a href={latest?.href || feed?.links?.setlists || "https://phish.net/setlists/phish"} target="_blank" rel="noreferrer">Full setlist ↗</a>
            <a href={feed?.links?.livePhish || "https://www.livephish.com/"} target="_blank" rel="noreferrer">Listen on LivePhish ↗</a>
          </div>
        </article>

        <article className={`${styles.card} ${styles.setlistCard}`}>
          <p>WHAT DID THEY PLAY?</p>
          <h3>{sets.length ? "Last show setlist." : "Last show highlights."}</h3>
          {sets.length ? (
            ...
            <div className={styles.songList}>
              {flatSongs.slice(0, 10).map((song, index) => (
                <span key={`${song}-${index}`}><b>{index + 1}</b>{song}</span>
              ))}
            </div>
          ) : (
            <span className={styles.emptyCopy}>Pulling the newest setlist from Phish.net.</span>
          )}
          <a href={latest?.href || "https://phish.net/setlists/phish"} target="_blank" rel="noreferrer">Open complete setlist ↗</a>
        </article>
